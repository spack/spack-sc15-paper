...
We begin with an example.%Let us explain our syntax with an example.
...
Spack also accepts spec syntax for conditionally patching packages.
Spack offers a {\tt patch} directive that uses specs as predicates in an
optional {\tt when} parameter.
These concise directives in the Python package ensure that very specific patches
are only applied to the Python source code when it is being built on Blue Gene/Q,
...
\end{lstlisting}
%
The {\tt when} clause is matched against the Python package's spec
to determine whether the patch will be applied.

Constraint tests can also be used directly within package source code.
For example, in the {\tt install} method, packages can test the versions of dependencies
using constraint syntax:
%
\begin{lstlisting}
  if '' in spec:
  elif '
...
In some cases, simple conditional logic


In our experience maintaining packages over time, we have found tha
